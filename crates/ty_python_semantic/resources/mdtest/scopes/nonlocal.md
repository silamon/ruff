...
into multiple steps. Local variable scoping is "foward-looking" in the sense that even a _later_
...
With `nonlocal` these examples work:
...
A global `x` doesn't work either; it has to be a function-like scope:
...
type declarations from outer scope don't apply to the inner one:
